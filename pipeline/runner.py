...
def process_chapter(
    book_id: str, previous_chapter_text: str, current_chapter_text: str
) -> str:
    ...
    hook = generate_hook(previous_chapter_text, current_chapter_text, metadata=metadata)

    for attempt in range(1, MAX_ITERATIONS + 1):
        passes, reason = critique_hook(
            hook, previous_chapter_text, current_chapter_text, metadata=metadata
        )
        if passes:
            break
        print(
        ...
        hook = rewrite_hook(hook, reason, previous_chapter_text, current_chapter_text, metadata=metadata)
